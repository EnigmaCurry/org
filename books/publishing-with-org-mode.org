...
headers. Start by resetting the visibility of all the headers
(=org-set-startup-visibility=):

My emacs config sets the default startup visibility to folded, so you
can always get back to a folded state:
