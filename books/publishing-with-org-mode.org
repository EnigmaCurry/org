...
Each window has a single *play/stop* control — stopping rewinds to the
start, so play always begins fresh. With no attributes a window starts
stopped on its first frame; press play to run it:
