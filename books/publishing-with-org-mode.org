...
A =shader= block embeds the built-in WebGL effect. With no attributes it
renders a small live window inline in the page:
...
For example, this block renders a window *and* shows its source:

#+attr_shortcode: :source true
#+begin_shader
