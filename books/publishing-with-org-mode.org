...
:PROPERTIES:
:EXPORT_FILE_NAME: shader-background
:EXPORT_HUGO_SLUG: shader-background
:END:

A =shader= block embeds the built-in WebGL effect in a small live window.
Each window has controls: *play/pause*, and a *reset/clear* button that
restarts the animation while playing or blanks the canvas while paused.
With no attributes a window starts paused on a static preview — press
play to run it:

#+begin_shader
#+end_shader

The =shader= block takes a few attributes:

- =:autoplay= — start animating on load instead of showing a paused
  preview.
- =:effect takeover= — render the effect full-screen behind the *whole*
  page using a single persistent canvas that survives navigation (it is
  only reconfigured, never recreated, as you move between pages), rather
  than as an inline window.
- =:hidden= — don't draw the inline window. Only useful together with
  =:effect takeover=, where the full-screen background is the visual.
- =:source= — also show the block's source below the window.

For example, this block autoplays *and* shows its source:

#+attr_shortcode: :autoplay true :source true
#+begin_shader
// A future release will compile custom GLSL from the block body.
// For now the built-in shader runs.
#+end_shader

And this one is a hidden full-screen takeover — look at the page
background rather than this spot, then navigate away to see it switch
off (the persistent canvas stays alive, just idle):
