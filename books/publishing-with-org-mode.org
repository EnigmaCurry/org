...
always begins fresh. With no body and no attributes, the built-in shader
runs (stopped on its first frame until you press play):

#+begin_shader
#+end_shader

Pass =:autoplay= to start it running on load instead of showing a
stopped preview:
...
in the window. The inputs are =iResolution= (vec3), =iTime= (float), and
=iAccent= (vec3, the current theme accent). When a block has a body its
source is shown alongside the window by default:
...
void mainImage(out vec4 fragColor, in vec2 fragCoord){
  ...
  vec3 col = mix(vec3(0.03, 0.03, 0.06), iAccent, v);
