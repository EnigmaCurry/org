...
** Shader background (experimental)
:PROPERTIES:
:EXPORT_FILE_NAME: shader-background
:EXPORT_HUGO_SLUG: shader-background
:END:

The site has a persistent WebGL background canvas that survives
navigation: it never restarts as you click between pages, because the
canvas lives outside the swapped content area and the soft-navigation
shell only reconfigures it. The background is *opt-in*: ordinary pages
render nothing (you just see the normal theme background), and a page
turns the effect on with a =shader= block — =ambient= for a calm field
or =takeover= for something stronger:

#+begin_src org
,#+attr_shortcode: :effect takeover
,#+begin_shader
,#+end_shader
#+end_src

With JavaScript disabled the background simply isn't shown and the page
reads normally — the effect is pure progressive enhancement. This page
requests the takeover effect:

#+attr_shortcode: :effect takeover
#+begin_shader
#+end_shader

Navigate to any other page and watch the effect switch off (the same
WebGL context stays alive, just idle), then return when you come back.
