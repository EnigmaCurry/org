...
the shader as you write. The =shader= block also takes a few attributes:

- =:autoplay= — start animating on load instead of showing a stopped
  preview.
- =:source false= — hide the source box for a block that has one.
- =:effect takeover= — render the built-in effect full-screen behind the
  *whole* page using a single persistent canvas that survives navigation
  (only reconfigured, never recreated), rather than as an inline window.
