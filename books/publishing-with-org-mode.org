...
The site has a persistent WebGL background that survives navigation: it
never restarts as you click between pages, because the canvas lives
outside the swapped content area and the soft-navigation shell only
reconfigures it. Most pages use a calm =ambient= field; a page can opt
into a stronger =takeover= effect with a =shader= block:
...
Navigate to any other page and watch it ease back to the ambient field —
the same WebGL context, reconfigured rather than recreated.
