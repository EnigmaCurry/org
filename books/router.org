...
#+attr_shortcode: :style secondary :title Run this on the Router VM
#+begin_run
systemctl enable --now nifty-router
#+end_run
...
#+attr_shortcode: :style primary :title Info
#+begin_notice
Despite the name, dnsmasq's main job is to provide DHCP service to the
LAN. It will also forward DNS queries to your local dnscrypt-proxy. In
addition, dnsmasq will resolve any host name found in the router VM's
=/etc/hosts= file.
#+end_notice
