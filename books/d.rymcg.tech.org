...
**** DON'T install Docker Desktop!
 ...
 2. Docker Desktop runs a VM to run the Docker daemon on localhost.
    ...
    instead.)
 3. Docker Desktop doesn't support host mode networking, so it
    wouldn't work with our Traefik configuration anyway.
...
#+attr_shortcode: :style tip :title Tip: vestigal default Docker context
#+begin_notice
There is now a vestigal Docker context named "default" left on your
system, and this context was originally used to manage the Docker
daemon of the local host. However, it will be of no use to you, since
the Docker daemon is now completely disabled on the workstation.
Furthermore, the "default" context cannot be deleted, so its best to
just ignore it (the =d.rymcg.tech= CLI won't even show it). In the
next couple of steps, you'll create new *remote SSH contexts* that
you'll use instead.
#+end_notice
