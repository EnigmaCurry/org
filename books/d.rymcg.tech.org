...
different server scenarios:
 ...
 1. A production, public Docker server on the Internet (eg. hosted at
    DigitalOcean, or any other VPS cloud, and/or bare-metal server).
 2. A production/staging Docker server on a private LAN (eg. a
    Raspberry Pi).
 3. A development Docker server in a Proxmox VM.

If you only care about a particular scenario, you may now want to skip
directly to the appropriate chapter.
...
The droplet has Debian installed on it, with only basic packages
installed. To install Docker, run :
...
You have now finished installation of a remote Docker host server,
configured via remote docker context on your workstation.
