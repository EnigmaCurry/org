...
# If you're on Arch Linux, you are good to go, because it tracks
...
# #+attr_shortcode: :style warning :title Docker tools on Fedora 40
# #+begin_notice
# On Fedora (40), the docker packages from the default repository are
# too old, especially the =docker-compose= package. You should only
# install it from the upstream (docker.com) package repository explained
...
#     instead.) Furthermore, the VM that it creates, runs from the same
#     user account as you normally use, so its storage system is not
#     secured.
...
#     Desktop 4.29+).
#  4. Docker Desktop does provide the same =docker= client tools, so it
#     will actually still work, if thats the package you prefer to
#     install. Just be sure to disable the VM that it creates by
#     default, you will /not/ need it!

# If you really want to run VMs on your workstation, read the Linux
# Workstation book's chapter [[/linux-workstation/kvm-libvirt][KVM / Libvirt]].
    
# *** Disable Docker daemon on your workstation

# Your workstation is the manager of /remote/ Docker hosts, so it should
# not run the Docker daemon (server) itself. It should only run the
# docker /client/.

# #+attr_shortcode: :style info :title Disable Docker on native host
# #+begin_notice
# If you installed Docker inside of a toolbox container, then you don't
# need to take any action (because systemd is not activated in toolbox).

# However, if you installed Docker tools on a native host, run the
# following commands to disable the Docker daemon:
...
# This context was originally used to manage the Docker daemon of the
# local host (=unix:///var/run/docker.sock=). However, this will be of
# no use to you now, since the Docker daemon is now completely disabled
# on your workstation. Unfortunately, the "default" context cannot be
# deleted, so its best to just ignore it (the =d.rymcg.tech= CLI won't
# even show it). In the next couple of steps, you'll create and activate
# new *remote SSH contexts* and you'll use those instead of the
# "default" context.
# #+end_notice
...
# #+begin_run
# sudo dnf install bash gettext openssl git xdg-utils jq sshfs curl \
...
# #+begin_run
# sudo pacman -S bash base-devel gettext git openssl apache xdg-utils \
...
# #+begin_notice
# By convention, *you should not change the  =ROOT_DIR= clone path*. It
# is intentionally placed in a vendor neutral path location for all to
# use. But, if you really want to change it, it should still work,
# regardless of where you put it. But watch out, as this may break
# documentation, and for some external projects that assume =ROOT_DIR=
# is using the conventional path. For compatability reasons, consider
# making a symlink pointing to your actual clone path:

# #+begin_run
# ## Only run this if you customized ROOT_DIR
# (set -e
# test -z "${ROOT_DIR}" && echo -e "\nROOT_DIR is unset. Exiting." && exit 1
# test "${ROOT_DIR}" == "${HOME}/git/vendor/enigmacurry/d.rymcg.tech" && \
#   echo -e "\nROOT_DIR is the default path: ${ROOT_DIR}" && \
#   exit 1
# mkdir -p ~/git/vendor/enigmacurry
# ln -s ${ROOT_DIR} ~/git/vendor/enigmacurry/d.rymcg.tech
# )
# #+end_run
# #+end_notice
...
# ** Traefik Scenarios
# :PROPERTIES:
# :EXPORT_HUGO_SECTION_FRAG: scenarios
# :END:
...
# This section will discuss deploying Traefik in a variety of scenarios.
# Each scenario will build upon the next, so you might consider
# following them in order, but you can also skip straight to the one you
# want, as each set of instructions is self-contained.
...
# *** Minimal Traefik
# :PROPERTIES:
# :EXPORT_FILE_NAME: minimal
# :EXPORT_HUGO_WEIGHT: 1000
# :END:

# This scenario covers the bare minimum Traefik configuration. All
# optional configuration will be skipped. This is the minimum config to
# serve a website on port 443, but only with temporary self self-signed
# TLS certificates ("Traefik Default").

# #+attr_shortcode: :style tip
# #+begin_notice
# This scenario is good for testing, but not for secure public access.
# #+end_notice
...
# #+attr_shortcode: :style warning
# #+begin_notice
# If you want to reset the existing config back to the defaults, run:

# #+begin_run
# d make traefik clean
# #+end_run
# #+end_notice
...
# Use your arrow keys to select only the menu items this tutorial does
# not tell you to skip. Select the first one and press the =Enter= key.
# To go back, press the =ESC= key.

# Once you go through all the options you need, it will have created and
# configured your =.env_{CONTEXT}= file for you.
...
#  * =websecure= (TCP port 443, https)
#  * =web= (TCP port 80, but only used as redirect to =websecure=)
...
# ***** Skip: Configure ACME (Let's Encrypt)
# Since you will use the default certificates, ACME should be disabled.
# ***** Skip: Configure TLS certificates and domains
# This config will use self-signed temporary "Traefik Default"
# certificates only.
# ***** Skip: Configure middleware (including auth)
# No middleware is needed for this scenario.
# ***** Install Traefik

# To install Traefik, you can simply choose the =Reinstall Traefik=
# option in the menu, or you can run that step all by itself from the
...
# setting.
# ***** Test Traefik by installing whoami

# Install [[/d.rymcg.tech/whoami][whoami]].

# Because of the fact you are using the default self-signed TLS
# ceritificates, your web browser will display an error that it cannot
# validate the authenticity of the server. You may bypass this error by
# adding an exception.

# An easier way to test is with =curl -k=:
...
# #+begin_stdout
# Name: default
# Hostname: 65ef62c6fa52
# IP: 127.0.0.1
# IP: ::1
# IP: 172.19.0.2
# RemoteAddr: 172.19.0.1:47974
# GET / HTTP/1.1
...
# X-Forwarded-Server: debian-dev
# X-Real-Ip: 192.168.122.1
# #+end_stdout

# *** Let's Encrypt TLS challenge
# :PROPERTIES:
# :EXPORT_FILE_NAME: vps-lets-encrypt-tls-challenge
# :EXPORT_HUGO_WEIGHT: 2000
# :END:

# This scenario covers a basic Traefik configuration for a public server
# (VPS). This uses [[https://letsencrypt.org/][Let's Encrypt]] to sign TLS certificates, which are
# valid for use in most web browsers. This scenario will use the
# *TLS-ALPN-01* challenge type.

# #+attr_shortcode: :style warning
# #+begin_notice
# This scenario is only good for /public/ websites and services. For
# (semi-)private servers, that you don't really want the world to know
# about yet, you should avoid this and use the DNS-01 challenge instead.
# *TLS-ALPN-01 will leak all of your domain names to [[https://letsencrypt.org/docs/ct-logs/][public CT logs]]* and
# you will have bots crawling your domains within seconds of certificate
# issuance.
# #+end_notice

# **** ACME TLS-ALPN-01 challenge type

# [[https://letsencrypt.org/docs/challenge-types/][ACME challenges]] are a type of procedure that authenticates your server
# to Let's Encrypt that you are the owner (controller) of your domain
# name system (DNS). This is important because it prevents other actors
# from requesting certificates for your domains.

# The "TLS challenge" (TLS-ALPN-01), is one of /two/ supported ACME
# Challenge types. TLS-ALPN-01 is the default type, which has some
# advantanges, and disadvantages.

#  * It /requires/ port 443 to be public. *It cannot work when port 443
#    is blocked.*
#  * It /only/ requires a valid DNS =A= record for the certificate
#    domains to be pointing to the IP address of your server. For a
#    public service, you would have needed to create this record anyway,
#    so this isn't really an extra requirement.
#  * Compared to other supported challenge type, DNS-01, it is much
#    easier and safer to configure, because it does not require the
#    storage of a sensitive API token for your DNS provider.
#  * A significant drawback is the limitation that it can only issue
#    certificates for an *explicit list of domain names*. No wildcards
#    (=*.example.com=) are allowed with TLS-ALPN-01. So you will need to
#    revisit your traefik config every time you add a new application
#    domain name.
#  * All Let's Encrypt challenge types participate in *public*
#    [[https://letsencrypt.org/docs/ct-logs/][Certificate Transparency
#    logs]]. Therefore, any domain name you create a certificate for is
#    publicly "leaked" via CT logs. If you can't use wildcard domains,
#    every explicit name you create will show up in the CT logs.
#    Therefore, TLS-ALPN-01 is actually the worst privacy wise. You will
#    have a higher likelyhood of bots crawling your services within
#    seconds after your name shows up in CT logs. However, this can be
#    mitigated simply by enabling sentry authorization, so only
#    authorized clients may access it.
...
# ***** Choose TLS-ALPN-01 ACME challenge
# #+begin_stdout
...
# #+begin_stdout
# Enter the main domain (CN) for this certificate
# : prod.example.com
# #+end_stdout
...
# certificate. The record for =whoami= is required for the next chapter,
# [[/d.rymcg.tech/whoami][Whoami]].
# #+end_notice

# #+attr_shortcode: :style tip
# #+begin_notice
# Make sure that each domain name that you entered on the certificate
# has a valid DNS =A= record, pointing to your Docker server's IP
# address.
# #+end_notice
...
# #+begin_run
# curl https://whoami.prod.example.com
# #+end_run
...
# RemoteAddr: 172.19.0.1:47974
# GET / HTTP/1.1
# Host: whoami.prod.example.com
# User-Agent: curl/8.2.1
# Accept: */*
# Accept-Encoding: gzip
# X-Forwarded-For: 192.168.122.1
# X-Forwarded-Host: whoami.prod.example.com
# X-Forwarded-Port: 443
# X-Forwarded-Proto: https
# X-Forwarded-Server: debian-dev
# X-Real-Ip: 192.168.122.1
...
# You can verify the certificate this way:

# #+begin_run
# WHOAMI=whoami.prod.example.com
# openssl s_client -connect "$WHOAMI:443" < /dev/null >/dev/null
# #+end_run
...
# verify return:1
# depth=0 CN = whoami.prod.example.com
# verify return:1
# DONE
# #+end_stdout
...
# :PROPERTIES:
# :EXPORT_FILE_NAME: vps-lets-encrypt-dns-challenge
# :EXPORT_HUGO_WEIGHT: 3000
# :END:

# This scenario covers a Traefik configuration for a public /or private/
# server. This uses [[https://letsencrypt.org/][Let's Encrypt]] to sign TLS certificates, which are
# valid for use in most web browsers. This scenario will use the
# *DNS-01* challenge type.

# #+attr_shortcode: :style info
# #+begin_notice
# This scenario is good for /public/ and /private/ websites and
# services.

# All Let's Encrypt challenges *leak domain names to [[https://letsencrypt.org/docs/ct-logs/][public CT logs]]*,
# however, with DNS-01 challenge we can use wildcards, so that only the
# root domain is leaked, not the exact hostnames.
# #+end_notice

# #+attr_shortcode: :style warning
# #+begin_notice
# This scenario offers a much more powerful feature set, but it requires
# storing a security sensitive API token, to control the active DNS
# server of your domain. You must take special steps to store this token
# only on a secure workstation and and Docker host.
# #+end_notice


# **** ACME DNS-01 challenge type

# [[https://letsencrypt.org/docs/challenge-types/][ACME challenges]] are a type of procedure that authenticates your
# Traefik instance to the Let's Encrypt API, proving that you are the
# owner (controller) of your domain name system (DNS). This is important,
# because it prevents other actors from requesting certificates for your
# domains.

# The "DNS challenge" (DNS-01), is one of /two/ supported ACME Challenge
# types. DNS-01 has some advantanges, and disadvantages.

#  * DNS-01 challenge works for servers that are behind restrictive
#    firewalls. No open ports are necessary.
#  * DNS-01 challenge allows wildcard certificate domains
#    (=*.example.com=), which means less work when you want to deploy a
#    new application, as long as it uses a valid sub-domain of the
#    existing certificate, you won't need to reconfigure traefik each
#    time.
#  * All Let's Encrypt challenge types participate in *public*
#    [[https://letsencrypt.org/docs/ct-logs/][Certificate Transparency
#    logs]]. Therefore, any domain name you create a certificate for is
#    publicly "leaked" via CT logs. However, if you only use a wildcard
#    domain (=*.example.com=) in your certificate, then only the root
#    domain name (=example.com=) will be leaked, and the bots that are
#    reading the CT logs would have to guess your exact sub-domains, in
#    order to crawl them.
...
# #+attr_shortcode: :style tip
# #+begin_notice
# For this install, choose =DNS-01=.

# The challenge type will let you use wildcard DNS records, and also
# works behind a firewall, but it requires storing a security token for
# your DNS API.
# #+end_notice
# ***** Skip: Configure ACME email address
...
# ***** Enter ACME DNS provider config
...
# #+attr_shortcode: :style tip
# #+begin_notice
# You may use any supported DNS provider of LEGO. This example uses the
# [[https://go-acme.github.io/lego/dns/digitalocean/][DigitalOcean provider]]. If you're using a different platform, you need
# to look up the supported [[https://go-acme.github.io/lego/dns/#dns-providers][DNS provider code]] in the LEGO docs, and type
# its name. The code is always one word, no spaces.
# #+end_notice
...
# #+attr_shortcode: :style info
# #+begin_notice
# Each supported DNS provider requires certain variables to be set, in
# order to pass the API token. In the [[https://go-acme.github.io/lego/dns/digitalocean/][LEGO docs for DigitalOcean]] it
# lists the variable name that it needs. This question is asking for the
# literal *name* of the variables specific to the DNS platform, which
# for DigitalOcean is: =DO_AUTH_TOKEN=. If your provider needs more than
# one variable set, you can enter them on additional lines, but
# DigitalOcean only needs one variable set, so it leaves the second line
# blank.
# #+end_notice

# ***** Enter ACME DNS API token
# Next it will ask you to enter the values for the variable names you
# just set. This is where you put *your actual API token* of your DNS
# platform.
# #+begin_run
...
# SANs (Subject Alternative Names). You can use this to list as many
# additional domain names that you want to be listed on the same
# certificate. In the special case of DNS-01, you are allowed to use a
# *wildcard domain* (=*.example.com=) for either the main CN or the
# SANS.
# #+end_notice
...
# Make sure that each domain name that you entered on the certificate
# has a valid DNS =A= record, pointing to your Docker server's IP
# address, including any wildcard domain.

# The next chapter will require the domain =whoami.prod.example.com=,
# and this will automatically be covered by the wildcard domain
# =*.prod.example.com=.
# #+end_notice
...
# depth=1 C = US, O = Let's Encrypt, CN = R3
# verify return:1
# depth=0 CN = *.prod.example.com
# verify return:1
# DONE
# #+end_stdout

# *** Step-CA TLS challenge
# :PROPERTIES:
# :EXPORT_FILE_NAME: step-ca-tls-challenge
# :EXPORT_HUGO_WEIGHT: 4000
# :END:
# This scenario covers a basic Traefik configuration for a public server
# (VPS). This uses [[https://smallstep.com/docs/step-ca/][Step-CA]] to sign off-grid TLS certificates. This
# scenario will use the *TLS-ALPN-01* challenge type.

# Step-CA is a self-hosted ACME provider, a root Certificate Authority.
# Step-CA is an alternative to Let's Encrypt, but they do serve
# difference purposes:

#  * Use Let's Encrypt if you need "valid" certificates trusted by web
#    browsers.
#  * Use Step-CA if you want fully self-hosted PKI, and your own
#    Certificate Authority. The certificates will not be validated by
#    your clients unless they add your root CA public key to their OS or
#    web browser trust store.

# #+attr_shortcode: :style warning
# #+begin_notice
# This scenario is only good for /private/ websites and services, as the
# certificates are considered "self signed", they will not be trusted by
# normal web browsers, unless you add it to the trust store.
# #+end_notice

# **** Create a Step-CA server

# Follow the chapter named [[/d.rymcg.tech/step-ca][Step-CA]] to create your own Step-CA powered
...
#    revisit your traefik config every time you add a new application
#    domain name.
#  * Step-CA does /not/ participate in
#    [[https://letsencrypt.org/docs/ct-logs/][Certificate Transparency
#    logs]].
...
# You must configure Traefik instance to trust the root certificate of
# your Step-CA instance.
...
# #+begin_run
# curl --cacert CA_CERT_FILE https://whoami.prod.example.com
# #+end_run
...
# #+begin_stdout
# depth=1 C = US, CN = ca.example.com
# verify return:1
# depth=0 CN = whoami.prod.example.com
# verify return:1
# DONE
...
# *** Step-CA DNS challenge
# :PROPERTIES:
# :EXPORT_FILE_NAME: step-ca-dns-challenge
# :EXPORT_HUGO_WEIGHT: 5000
# :END:
# TODO
# *** Wireguard layer 7 proxy
# :PROPERTIES:
# :EXPORT_FILE_NAME: wireguard-layer-7-server
# :EXPORT_HUGO_WEIGHT: 6000
# :END:
# TODO
# *** Wireguard layer 7 client
# :PROPERTIES:
# :EXPORT_FILE_NAME: wireguard-layer-7-client
# :EXPORT_HUGO_WEIGHT: 7000
# :END:

# TODO
# *** Wireguard layer 4 client
# :PROPERTIES:
# :EXPORT_FILE_NAME: wireguard-layer-4-client
# :EXPORT_HUGO_WEIGHT: 8000
# :END:
...
# After you install Traefik, whoami should be the very next application
...
# template for all the other apps provided by d.rymcg.tech. Whoami can
# be a good base template for creating your own d.rymcg.tech enabled
# applications.
...
# preferred web browser to the given application's URL. There are
# various reasons why this may not work, so be prepared in that event to
# know that you may simply copy the URL that it prints, and copy and
# paste that into your browser: =https://{WHOAMI_TRAEFIK_HOST}=
# #+end_notice
...
# This sets the acceptable IP address ranges for incoming requests. It
# is a comma separated list of [[https://en.wikipedia.org/wiki/CIDR#CIDR_notation][CIDR formatted networks]].
...
# * Step-CA
# :PROPERTIES:
# :EXPORT_HUGO_SECTION_FRAG: step-ca
# :END:

# ** Step-CA
# :PROPERTIES:
# :EXPORT_FILE_NAME: _index
# :EXPORT_HUGO_WEIGHT: 2040
# :END:

# [[https://smallstep.com/docs/step-ca/][Step-CA]] is a secure, online, self-hosted Certificate Authority (CA).
# Its purpose is to issue (sign) X.509 (TLS) certificates, and to
# securely store the private key for the root CA. It can issue
# certificates manually using the =step-cli= tool, or Traefik can
# request them automatically via the ACME protocol.
...
# *** Select your Docker context

# Running your own Certificate Authority should be a careful
# undertaking. Ideally, your Step-CA instance should run on its own
# dedicated Docker (virtual) machine. That way, it can stay up long
# term, as part of your personal cloud infrastructure, and not be tied
# to the lifecycle of any other application you run in your domain.
...
# If you are creating a new Traefik instance, follow the [[/d.rymcg.tech/traefik-proxy/scenarios/minimal][Minimal
# Traefik]] install. You do not need anything extra like ACME, nor any
# TLS certificates. Step-CA does all that for itself.

# Step-CA can also be installed on an existing Traefik instance, even
# one already configured to use Let's Encrypt. Traefik is configured to
# do TLS passthrough directly to Step-CA, which handles its own TLS
# certificate.
...
# d make step-ca up service=step-ca
# #+end_run
...
# #+attr_shortcode: :style tip
# #+begin_notice
# If this is not the /very first time/ you've tried running this, and you
# can't find the password in the log, and you want to start completely
# fresh, destroy the instance, and then try again:

# #+attr_shortcode: :style warning
# #+begin_run
# d make step-ca destroy
# d make step-ca up service=step-ca
# #+end_run
# #+end_notice
...
# because our config container requires this file to already exist (it
# wants to modify an existing config file), and since its supposed to
# run before the step-ca container, we must run it manually, one time
# only. This ensures that =ca.json= now exists in the config volume, and
# now the config container can fully manage this file going forward. In
# other words: the config values you set in your =.env_{CONTEXT}= file
# will only take affect after the SECOND time the container boots.
# #+end_notice
...
# #+begin_run
# d make step-ca install wait
# #+end_run

# Traefik will wait for the service healthcheck to complete, before
# serving it. The =wait= target will block until this occurs.


# ** Create Certificates
# :PROPERTIES:
# :EXPORT_FILE_NAME: certificates
# :END:
# *** Setup step-cli
...
# [[https://smallstep.com/docs/step-cli/installation/][step-cli]], which you should install on your workstation, using your
# package manager, or according to those instructions.

# #+attr_shortcode: :style info
# #+begin_notice
# Some packages install only the binary named step-cli. step-cli is just
# another name for step. However, most of the Step-CA documentation uses
# the step name in place of step-cli. So if your package doesn't install
# step (Arch Linux is this way), you can add =alias step=step-cli= to your
# =~/.bashrc= or wherever you put that stuff on your workstation. You can
# also change the STEP variable at the top of the Makefile to use to
# point to any name/path.
# #+end_notice

# Once you've install the =step-cli= program, you need to configure it
# to use your server:

# #+begin_run
# d make step-ca client-bootstrap
# #+end_run
...
# Once completed, it will create three new files on your worksation:

#  * =certs/{DOMAIN}.crt= - This is the public certificate file for your
#    host, along with the full public CA certificate chain.
...
# You can inspect the certificate file, and gather important details
# about it:
...
#  * Here is the [[https://wiki.archlinux.org/title/TLS#Trust_management][trust management document]] on the Arch Linux wiki. It is
#    also applicable to most other Linux distributions.
...
# ** Change Root CA Passphrase
# :PROPERTIES:
# :EXPORT_FILE_NAME: root-ca-passphrase
# :END:
...
# In your Traefik instance, you will want to configure ACME to point to
# your Step-CA instance. When you run =d make traefik config=, choose
# =Configure ACME=, choose =Step-CA= and you will be prompted to enter
# the ACME endpoint URL, which is:

# #+begin_src
# https://ca.example.com/acme/acme/directory
# #+end_src

# (replace =ca.example.com= with your own domain name.)

# ** Mutual TLS
# :PROPERTIES:
# :EXPORT_FILE_NAME: mtls
# :END:
...
# In the =d make traefik config= menu, choose =Configure Certificate
...
# #+begin_run
# ## SAFE: Pass the root CA cert directly to curl:
# curl --cacert ${ROOT_STEP_CA_CERT} ${WHOAMI_URL}
# #+end_run

# #+attr_shortcode: :style warning :title Add root CA to your system trust store
# #+begin_notice
# You can also add the root CA to your system trust store, so most
# programs will trust the CA by default. If you do this, you need to be
# extra sure that you have secured the private key, as this is a good
# way to get pwned.
...
# #+begin_run
# ## SAFE: Uses your system trust store, which now contains your CA:
# curl ${WHOAMI_URL}
# #+end_run
