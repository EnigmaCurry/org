...
#+attr_shortcode: :style secondary :title Docker on Fedora 40
#+begin_notice
On Fedora 40 I am still evaluating whether the docker packages
provided by the Fedora package repositories is OK or not. I know that
redhat made a lot of changes. You probably want to install from the
official Docker package repository instead.

#+attr_shortcode: :title Install Docker client tools on Fedora
#+begin_run
...
#+begin_notice
If you installed Docker tools only inside a toolbox container, then
you don't need to take any action (because systemd is not activated in
toolbox).
