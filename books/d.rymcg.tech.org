...
*TLS-ALPN-01 will leak all of your domain names to public CT logs* and
 ...
 * A significant drawback is the limitation that it can only issue
   ...
   (=*.example.com=) are allowed with TLS-ALPN-01.
...
All Let's Encrypt challenges *leak domain names to public CT logs*,
...
#+attr_shortcode: :style warning
#+begin_notice
This scenario requires storing a security sensitive API token to
control the DNS of your domain. You must take special steps to store
this token in your .env file, only on a secure workstation.
#+end_notice
 ...
 * All Let's Encrypt challenge types participate in *public*
   [[https://letsencrypt.org/docs/ct-logs/][Certificate Transparency
   logs]]. Therefore, any domain name you create a certificate for is
   publicly "leaked" via CT logs. However, if you only use a
   "wildcard" domain (=*.example.com=) in your certificate, then only
   the root domain name will be leaked, and the bots that are reading
   the CT logs would have to guess your exact hostnames in order to
   crawl them.
