...
:END:
This scenario covers a basic Traefik configuration for a public server
(VPS). This uses [[https://smallstep.com/docs/step-ca/][Step-CA]] to sign TLS certificates, which are valid for
use in most web browsers. This scenario will use the *TLS-ALPN-01*
challenge type.

Step-CA is a self-hosted ACME provider, a root Certificate Authority,
to sign TLS certificates. Step-CA is an alternative to Let's Encrypt,
but they serve difference purposes:
...
certificates are considered "self signed", they will not be trusted in
normal web browsers.
#+end_notice
