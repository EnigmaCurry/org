...
It should be the very first application you install after Traefik. You
will see later on that all of the applications provided by
d.rymcg.tech are configured and managed similar to this one. Use this
as a learning experience. If you can accomplish gettting whoami
installed, you'll be able to install anything.

** What is Whoami?

Whoami is a web application useful for testing that Traefik is working
as expected. Its sole purpose is to show a web page that prints the
HTTP headers that were sent to it. This can be useful to verify that
the application is capable of serving requests, and that all of the
