...
After Traefik, it should be the very first application that you
install. It can help you test whether or not your Traefik installation
is functioning properly.

Later on, you will come to realize how similar all of the other
applications provided by d.rymcg.tech are to whoami, by the common
methods that they are configured and managed. *You should study the
configuration of whoami.* Because of its simplicity, it reduces the
complexity of its core components. Whoami can be a good base template
for creating your own d.rymcg.tech enabled applications.

Use this as a learning experience. If you can accomplish gettting
whoami installed, you'll be able to install any of the other
applications provided by d.rymcg.tech. If you learn how it works,
you'll be able to use this template to make your own d.rymcg.tech
apps.

** What is Whoami?

Whoami is a web application that simply outputs the request headers it
receives. It useful for end-to-end testing that Traefik is working as
expected. Its sole purpose is to show a web page that prints the HTTP
headers that were sent to it. This can be useful to verify that the
...
[[https://github.com/traefik/whoami?tab=readme-ov-file#whoami][See the upstream whoami documentation here.]]

** Default config file (.env-dist)

Every application provided by d.rymcg.tech, including whoami, has a
default configuration file named =.env-dist=. This config file is used
as a template, which is copied whenever you configure a new instance
of the application. The =.env-dist= file should never be edited, as it
is always the default configuration. Each instance has its own copy,
and it is in the copy that you have the opportunity to change those
defaults.

** Configure Whoami

To configure whoami, run:

#+begin_src
d make whoami config
#+end_src

This process copies the default config file (=.env-dist=) and creates
a new config file, specifically for this instance of the application:
(=.env_prod_default=).

*** The Whoami env file

Every application instance has a unique config file, and the =config=
target creates it. The first thing you see when running =d make whoami
config= is the name of the environment (config) file that it creates:

#+begin_stdout
Configuring environment file: .env_prod_default
#+end_stdout

All of the whoami .env files are stored in the whoami directory
itself, which is in =~/git/vendor/enigmacurry/d.rymcg.tech/whoami= (by
default). The naming of the file (=.env_prod_default=) tells us two
things:

 * =prod= is the name of the active Docker context it was created for.
 * =default= is the name of the instance of whoami (default is the
   name of the default instance, but if the instance was named =foo=
   the file would be named =.env_prod_foo= instead.).

You can have multiple config files, each one is for a separate Docker
context and application instance.
...
The next question is about sentry authorization, which configures what
type of authentication you want to use, and what clients to allow in
through the door.
...
You can choose any of the choices, and they will configure your application differently:

 * If you select =No=, then sentry authorization will be turned off.
   Any client will be able to access the application.
 * If you select =Yes, with HTTP Basic Authentication=, the
   application will require all clients to enter a username/password
   into a dialog presented by the web browser. Client who enter an
 ...
 * If you select =Yes, with OAuth2=, the application will require all
   ...
   that the application is configured for.
...
*** Edit the config file by hand if necessary
...
This will automatically open your default text editor to the whoami
config file for the current context/instance.
