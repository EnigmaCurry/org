...
:PROPERTIES:
:EXPORT_HUGO_SECTION_FRAG: domain-infrastructure
:END:
...
#+attr_shortcode: :title Notice
#+begin_notice
(The domain name =ssh.prod.example.com= should automatically point to
your VPS if you setup the wildcard DNS entry (=*.prod.example.com=).
The SSH config settings =ControlMaster=, =ControlPersist=, =ControlPath=
adds SSH connection multi-plexing, and will make repeated
logins/docker commands faster.)
#+end_notice
...
#+attr_shortcode: :title Run this on your Fedora workstations as root
#+begin_run
dnf install bash gettext openssl git xdg-utils jq sshfs curl \
            inotify-tools httpd-tools make wireguard-tools
#+end_run

#+attr_shortcode: :title Run this on your Debian workstations as root
#+begin_run
...
#+attr_shortcode: :title Run this on your Arch Linux workstations as root
#+begin_run
...
#+begin_stdout
? This command can help create a new SSH config and Docker context. Proceed? (Y/n)
...
You can view, and select from, all of the configured Docker contexts on
your workstation:
