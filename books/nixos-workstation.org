...
#+begin_notice
If you *do* want to use the official NixOS graphical installer instead,
follow the [[https://github.com/EnigmaCurry/sway-home/blob/master/NIXOS.md][NIXOS.md guide]] from the [[https://github.com/EnigmaCurry/sway-home][sway-home]] repository,
specifically the section titled /Option B/. You still end up with the
same per-host =~/nixos= repo: after the graphical install you run the
same =setup-host= tool (straight from GitHub) with its =--adopt= flag,
which reuses the installer's existing partitioning instead of disko.
#+end_notice
