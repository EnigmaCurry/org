...
A [[https://solokeys.com/][Solokey]] (or any FIDO2/U2F security key) can stand in for your
password when you run =sudo=: you touch the key instead of typing a
passphrase. This chapter sets that up on NixOS.
...
We make the key *sufficient* for =sudo= rather than *required*:

 - *sufficient* (this chapter) -- a key touch authorizes =sudo= /instead
   of/ a password. If no key is present, or it fails, =sudo= falls back
   to asking for your password. You therefore *cannot lock yourself out*
   by losing a key -- the password still works.
 - *required* -- a key touch /and/ a password, i.e. real two-factor
   =sudo=. This is stricter, but a lost/forgotten key now blocks =sudo=
   entirely. Switching to it is a one-word change, shown at the end.
...
#+begin_src nix
  # --- Optional: Solokey (FIDO2) as sudo authentication ------------------
  # Touch a registered Solokey *instead* of typing a password for sudo.
  # `control = "sufficient"` => the key REPLACES the password; if no key
  # is present sudo falls back to the password, so you can't lock yourself
  # out. Change it to "required" for true 2FA (key AND password).
  #
  # Paste the line from `pamu2fcfg` (see the Register chapter) below:
  # environment.etc."u2f_keys".text = ''
  #   ryan:hQ2k...,es256,+presence:tZ9p...,es256,+presence
  # '';
  # security.pam.u2f = {
  #   control  = "sufficient";                       # key instead of password
  #   settings = { cue = true; authfile = "/etc/u2f_keys"; };
  # };
...
What each piece does:
 ...
 - =control = "sufficient"= is the key-instead-of-password behavior.
 - =security.pam.services.sudo.u2fAuth = true= is what scopes this to
   =sudo=. We set it *per-service* on purpose.
...
into the =u2f_keys= text, then test it *without* making it permanent:
...
=sudo= succeeds with *no password*:

#+begin_stdout
Please touch the device.
#+end_stdout

Unplug the key and try again: because the rule is only =sufficient=,
=sudo= falls back to asking for your password, exactly as before.

** Two-factor instead (required)

To demand the key *and* the password -- real two-factor =sudo= -- change a
single word, =control=, from =sufficient= to =required=:

#+begin_src nix
  security.pam.u2f = {
    control  = "required";                          # key AND password
    settings = { cue = true; authfile = "/etc/u2f_keys"; };
  };
#+end_src
...
#+attr_shortcode: :style warning
#+begin_notice
With =required=, a lost or forgotten key blocks =sudo= entirely -- the
password alone is no longer enough. Register a backup key first, keep a
root shell open while you test (=admin test=, not =admin upgrade=), and
remember you can always reboot back to the previous generation.
#+end_notice

** Turning it off

Reverting is symmetric: re-comment the block (or set =u2fAuth = false=),
commit, and =admin upgrade=. The PAM rule disappears from the
regenerated =/etc/pam.d/sudo= and =sudo= goes back to password-only --
nothing left behind in =/etc= to clean up by hand.
