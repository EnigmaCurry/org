...
flake lock=. When it finishes it prints the exact apply command for
your hostname.

Review =~/nixos/config.nix= first (add an SSH key if none was found,
flip any profile toggles), then apply with =admin upgrade= -- this is
the step that *replaces* the stock configuration:
