...
continue at [[/nixos-workstation/first-login][First login to sway-home]].
...
* First login to sway-home
:PROPERTIES:
:EXPORT_FILE_NAME: first-login
:EXPORT_HUGO_SLUG: first-login
:EXPORT_HUGO_WEIGHT: 2500
:END:

Once sway-home is installed -- either via the custom ISO's =setup
install= or via the =setup-host= adopt flow on a stock install --
reboot the machine (or at minimum start a fresh login shell) so the
sway-home shell environment loads. Then log in as the user you
created.

The first interactive shell under sway-home runs a one-time
first-run prompt that sets up your git identity. It asks for:

- =user.name= -- the name that will appear on your commits
- =user.email= -- the email address for your commits
- =init.defaultBranch= -- your preferred default branch name (e.g.
  =main= or =master=)

It shows the values back to you and asks for confirmation before
writing them to =~/.config/git/config.local=. That file is
gitignored by the sway-home shared config, so your identity stays
local to this machine and never gets committed to the =~/nixos=
repo.

After you answer the prompt once it won't ask again on subsequent
logins. If you need to change any of these later, edit
=~/.config/git/config.local= directly.
...
With the first-login prompt out of the way, the more important habit
starts now: your machine's entire configuration lives in its own git
