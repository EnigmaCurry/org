...
#+begin_mermaid
---
title: Docker admin over SSH
---

graph TD;
    classDef dottedBox stroke-dasharray: 5 5;
    Workstation[Personal Workstation] -->|SSH| RaspberryPi[Raspberry Pi - Docker admin]
    RaspberryPi -->|SSH| Sentry[Sentry]
    RaspberryPi .->|SSH| Docker2[Docker 2, 3, etc.]
...
[[https://github.com/fail2ban/fail2ban?tab=readme-ov-file#readme][Fail2Ban]] scans log files and bans IP addresses that have too many
authentication attempts. Fail2Ban can be configured to monitor the
host SSH server and the Traefik access log.
...
SSHD_BAN_TIME=10m \
SSHD_FIND_TIME=10m \
SSHD_ALLOW_IPV6=true \
TRAEFIK_ACCESS_ENABLED=true \
TRAEFIK_ACCESS_MAX_RETRY=5 \
TRAEFIK_ACCESS_BAN_TIME=10m \
TRAEFIK_ACCESS_FIND_TIME=10m \
TRAEFIK_ACCESS_ALLOW_IPV6=true \
sentry fail2ban
#+end_run
