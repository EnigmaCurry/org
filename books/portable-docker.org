...
get the network up, have it resume all of its previous functions,
and even use the same IP address as before. It needs to be plug and
...
To accomplish this feat, we need to setup another server that has a
static IP address and accessible from the public internet. I use a
DigitalOcean droplet, which is a type of Virtual Private Server (VPS),
which exists in the public cloud. It is always on and anyone in the
world can connect to it (via HTTP). However, the droplet does not
serve any applications by itself; it only creates the public Traefik
proxy which forwards incoming connections (ingress) through a backdoor
VPN connection. The Raspberry Pi re-establishes this connection
automatically whenever it comes online.
...
how you follow the good DevOps practice, summed up by the metaphor:
"Treat your infrastructure like cattle, not like pets." However, this
strategy has the drawback that if you lose your workstation, it makes
it diffcult to adminster the server (you would have to recreate your
=.env= files from backup, or from scratch). So in this book, we will
make an important compromise and an exception to the normal rule.
