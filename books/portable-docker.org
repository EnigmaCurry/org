...
The installation process for Docker and d.rymcg.tech is fully
automated via bash environment variable and a downloaded script.

#+attr_shortcode: :style info :title Info
#+begin_notice
Before running the command below, you should edit it to set a custom
=ROOT_DOMAIN=, which is to be a dedicated sub-domain assigned to the
Pi. The =ALIASES= variable is already preset with the aliases you'll
be needing for this book: =pi= and =sentry=.
#+end_notice
...
*Debian*-derived system (including Raspberry Pi OS).

If you are wary of running curlbombs (you should be) you may want to
download and review the script ([[https://github.com/EnigmaCurry/d.rymcg.tech/blob/master/_scripts/bootstrap_sworkstation.sh][bootstrap_sworkstation.sh]]) and then
run it manually:
...
These are the manual steps you can take if you do not wish to run [[/portable-docker/set-up-raspberry-pi/install-docker-and-d.rymcg.tech/][the
automated script]].
