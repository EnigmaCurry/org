...
#+begin_notice
You should edit the command above and set a custom =ROOT_DOMAIN=
before running it, which is to be a dedicated sub-domain assigned to
the Pi. The =ALIASES= variable is already preset with the aliases you'll
be needing for this book: =pi= and =sentry=.
#+end_notice

#+attr_shortcode: :style tip :title Tip
#+begin_notice
The script is designed only to be run on a freshly installed
Debian-based system (including Raspberry Pi OS).

If you are wary of running curlbombs (you should be) you may review
the script ([[https://github.com/EnigmaCurry/d.rymcg.tech/blob/master/_scripts/bootstrap_sworkstation.sh][bootstrap_sworkstation.sh)]] and then copy and paste the
entire contents into the shell.

If you would rather set things up manually, you may skip this section
and follow the step by step instructions [[/portable-docker/appendix/manual-installation/][in the appendix]].
#+end_notice
...
d.rymcg.tech Docker context of the local host, named =pi=, but you can
set this to any name you like.

**** CONTEXT

#+attr_shortcode: :style tip :title Tip
#+begin_notice
You only need to set =CONTEXT= if you want to name the Docker context
something other than =${ALIASES}=
...
default name is only a suggestion, and each app can set whatever
...
[[https://hub.docker.com/_/docker/][Docker-in-Docker]], [[https://github.com/openfaas/faasd/][Faasd]], or any image that runs systemd:

#+attr_shortcode: :style secondary :title Run this on your Raspberry Pi
#+begin_run
## Enable sysbox only if required:
export SYSBOX=true
#+end_run
