...
get the network up, and have it resume all of its previous functions,
and even using the same IP address as before. It needs to be plug and
play, with minimal disruption to the clients that expect this server
to be online and available, no matter what kind of internet connection
I have it connected to, it should be quick to get this server back online.

To accomplish this feat, we need to setup another server that has a static IP
address, accessible from the public internet. I use a DigitalOcean
droplet, which is a type of Virtual Private Server (VPS), which exists
in the public cloud. It is always on, and anyone in the world can
connect to it (HTTP). However, the droplet does not serve any
applications by itself, it only creates the public Traefik proxy
(ingress), which forwards incoming connections through a backdoor VPN
connection, that the Raspberry Pi automatically re-establishes when it
comes online.

[[/img/portable-docker/vpn-string-along.webp]]

The Raspberry Pi should acquire whatever kind of internet connection
is locally available by DHCP, so it won't have a static IP address
yet. Once it's online, it creates a VPN connection to the droplet via
[[https://www.wireguard.com/][WireGuard]]. Once connected to the VPN, the Pi is assigned a static
private IP address that only the droplet can communicate with.

[[/img/portable-docker/vpn-diagram.webp]]

The cloud server (DigitalOcean droplet) forwards incoming connections
from the internet to the Raspberry Pi, over the WireGuard VPN
connection. Traefik is used on the cloud server to create routes for
your services. Traefik supports the following route types:

 * HTTP Layer 7 (Routes a domain name [TLS SNI] over port 443)
 * TCP Layer 4 (Routes an IP address + TCP Port # to a specific
   backend host IP address on a specific TCP Port #)
 * UDP Layer 4 - TODO (unimplemented but doable)

This lets you route virtually any kind of service, from the public
cloud, to your Raspberry Pi sitting on your desk.

*** d.rymcg.tech

[[https://github.com/EnigmaCurry/d.rymcg.tech][d.rymcg.tech]] is a configuration manager for Docker, as well as a
collection of open source web services, with Docker Compose wrappers,
and config templates. It contains an extensive configuration wizard
for [[https://github.com/EnigmaCurry/d.rymcg.tech/tree/master/traefik#readme][Traefik]], with an idiomatic [[https://github.com/EnigmaCurry/d.rymcg.tech/tree/master#command-line-interaction][command line tool]], which makes Docker
Compose projects both easy to document, as well as to use.

Traditionally, d.rymcg.tech suggests to install itself /on a separate
workstation/, and to never actually login to your Docker server, but
rather remotely control it from your workstation. This is how you
follow the good DevOps practice, summed up by the metaphor: "Treat
your infrastructure like cattle, not like pets." However, this
strategy has the drawback that if you lose your workstation, it makes
it diffcult to adminster the server. So in this book, we will make an
important compromise and exception to the normal rule.

In this book, d.rymcg.tech will be installed /to the same server/ that
runs Docker. This way, everything you need is installed on the
Raspberry Pi itself. To gain access, from any other workstation, you
only need to setup an SSH client, along with an authorized key. You
can login to the Pi and perform all maintainance locally, without
needing to manage a separate workstation. Your portable Docker server
is therefore a /pet/ Docker server, with your "workstation" being
located in the home directory of the =pi= user, colocated on the
Raspberry Pi itself.
...
You will also need to generate an additional API token for the sentry
droplet (you could use the same API token from above, but its best to
create a new token for each new instance).

 * Create an additional token, this one named =sentry.example.com
   ACME= or similar.
 * Copy this token to a temporary buffer/notepad as well, you'll need
   it when setting up the sentry droplet.
 ...
 * *Read the directions that come with the Geeekpi case*, the rest of
 ...
 * Click =Edit Settings=.
   ...
   * On the =General= tab:
     ...
     * Enter a username and password (This book uses the username
       =pi=).
     * Optionally setup the Wi-Fi (I just use ethernet instead).
...
config is somewhat temporary, and once DNS is setup later on, it can
...
TRAEFIK_WEBSECURE_ENTRYPOINT_HOST: Enter the host ip address to listen on (0.0.0.0 to listen on all interfaces) (eg. 0.0.0.0)
: 0.0.0.0
TRAEFIK_WEBSECURE_ENTRYPOINT_PORT: Enter the host port to listen on (eg. 443)
: 443
 ...
 * Create another =A= record, for the wildcard of the sentry:
   * Hostname: enter the same name as before but prepend =*.= in front
     of it (eg. if the server is named =sentry=, create a record for
...
TRAEFIK_VPN_HOST: Enter the public Traefik VPN hostname (eg. vpn.example.com)
: sentry.example.com
TRAEFIK_VPN_SUBNET: Enter the Traefik VPN private subnet (no mask) (eg. 10.13.16.0)
: 10.13.16.0
TRAEFIK_VPN_ADDRESS: Enter the Traefik VPN private IP address (eg. 10.13.16.1)
: 10.13.16.1
TRAEFIK_VPN_PORT: Enter the Traefik VPN TCP port number (eg. 51820)
: 51820
...
TRAEFIK_VPN_CLIENT_INTERFACE_ADDRESS: Enter the wireguard client Interface Address (eg. 10.13.16.2)
: 10.13.16.2
...
TRAEFIK_VPN_CLIENT_INTERFACE_LISTEN_PORT: Enter the wireguard listen port (eg. 51820)
: 51820
...
TRAEFIK_VPN_CLIENT_PEER_ALLOWED_IPS: Enter the Peer AllowedIPs (eg. 10.13.16.1/32)
: 10.13.16.1/32
#+end_stdout
...
#+begin_stdout
IMMICH_TRAEFIK_HOST: Enter the Immich domain name (eg. immich.example.com)
...
#+begin_stdout
YOURLS_TRAEFIK_HOST: Enter the YOURLS domain name (eg. yourls.example.com)
