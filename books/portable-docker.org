...
You'll also create and configure a public cloud server (eg.
DigitalOcean droplet), whose sole purpose is to host a VPN (WireGuard)
...
Press =ESC= three times to back out to the main menu and quit the
program.

*** Reinstall traefik using the new config
...
Once installed press =ESC= to quit the config tool.

You may also run the install command from the terminal:
...
Press =ESC= to quit the config tool.
