...
#+attr_shortcode: :style tip :title You can use any Linux computer instead.
#+begin_notice
To follow this book, you don't specifically require a Raspberry Pi.
You can replace it with any Linux computer (AMD64 or ARM64). The
instructions for installing its dependencies assume it to be running
Debian Linux, but this is not a requirement either if you can read
between the lines.
#+end_notice
