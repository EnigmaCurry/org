...
it only creates the public [[https://doc.traefik.io/traefik/][Traefik]] proxy (ingress) which forwards
incoming connections through a backdoor VPN connection to the
Raspberry Pi. The Pi automatically initiates this connection to the
sentry whenever it boots and becomes online.
...
yet. Once it's online, it creates a VPN connection to the sentry via
