...
If you don'y have =whois= installed, you can use [[https://www.registry.google/whois-lookup/][the web version provided by google]].
...
droplet (you could reuse the same API token from above, but its best to
 ...
 * Copy this token to the same temporary buffer/notepad as before,
   you'll need this second token when setting up the sentry droplet.
