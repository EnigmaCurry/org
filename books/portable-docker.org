...
can replace it with any Linux computer or virtual machine (AMD64 or
ARM64). The instructions provided for installing the required packages
assumes your machine to be running a variant of *Debian Linux* (such
as Debian, Raspberry Pi OS, Ubuntu, etc.), but this is not a
requirement either, as long as your platform supports Docker, and you
can follow the [[/portable-docker/appendix/manual-installation/][manual installation steps]].
#+end_notice
...
#+begin_notice
If you are not using a Raspberry Pi, you can skip to the [[/portable-docker/set-up-raspberry-pi/install-docker-and-d.rymcg.tech/index.html][Install
d.rymcg.tech]] section
#+end_notice
...
:PROPERTIES:
:EXPORT_FILE_NAME: 000250-install-docker-and-d.rymcg.tech
:EXPORT_HUGO_WEIGHT: 250
:EXPORT_HUGO_SLUG: install-docker-and-d.rymcg.tech
:END:

The installation process for [[https://docs.docker.com/engine/install/][Docker]] and [[https://github.com/enigmacurry/d.rymcg.tech][d.rymcg.tech]] can be automated
by the following Bash script.

#+attr_shortcode: :style info :title Info 
#+begin_notice
Before running the command below, you should edit the environment
variables defined at the top:

 * =ROOT_DOMAIN= needs to be a dedicated sub-domain name assigned to
   the Pi (e.g., =pi.example.com=).

 * =ALIASES= is already preset with the aliases you'll be needing for
   this book: =pi= and =sentry=. You may customize the names here if
   you wish.
#+end_notice
...
Once the script has finished, log out from the Pi and log back in. You
are done, and you may proceed to the next section ([[/portable-docker/set-up-raspberry-pi/install-traefik/index.html][Install Traefik]]).

#+attr_shortcode: :style tip :title Tip
#+begin_notice
If you don't like running automatic scripts, you can follow the [[/portable-docker/appendix/manual-installation/index.html][manual
installation steps]] listed in the appendix.

If you are wary of running curlbombs (you should be) you may want to
download and review the script ([[https://github.com/EnigmaCurry/d.rymcg.tech/blob/master/_scripts/bootstrap_sworkstation.sh][bootstrap_sworkstation.sh]]) and run it
manually:
...
This script is designed only to be run on a freshly installed
*Debian*-derived system (including Raspberry Pi OS). [[https://github.com/EnigmaCurry/d.rymcg.tech/blob/master/_scripts/bootstrap_sworkstation_fedora.sh][There is another
script for machines running Fedora]].
#+end_notice
...
d.rymcg.tech Docker context of the local host (named =pi=) and another
alias for the remote sentry (named =sentry=), but you can rename these
aliases to any names you like. =ALIASES= is a comma separated list of
alias names to create: =pi,sentry=.

**** ROOT_DOMAIN

=ROOT_DOMAIN= should be a unique dedicated sub-domain assigned to your
Docker server (e.g., =pi.example.com=). This becomes the default root
domain for every app you install (e.g., =whoami.pi.example.com=.) The
default name is only a suggestion and each app can set whatever domain
name they want.

#+attr_shortcode: :style info :title ROOT_DOMAIN
#+begin_notice
...
records, e.g., =*.pi.example.com=, you only have to setup DNS records
once per Docker host, even if you install more apps later. In this
book, =pi= is just a short identifier to make these URLs unique to a
...
the default =SSH_HOST= is =localhost=. You don't need to change this
...
=SSH_HOST= (=localhost=). You don't need to set this unless you need
...
If you need to grant special privileges to your containers, you may do
so with [[https://github.com/nestybox/sysbox#readme][Sysbox]]:

#+attr_shortcode: :style tip :title Tip
#+begin_notice
This is only required for some special cases, e.g., rootless
[[https://hub.docker.com/_/docker/][Docker-in-Docker]], [[https://github.com/openfaas/faasd/][Faasd]], or any image that runs systemd. Set
=SYSBOX=true= to enable it.

You may also export =SYSBOX_URL= to point to a particular [[https://github.com/nestybox/sysbox/releases][release
...
sequenceDiagram
    ...
    participant Whoami as Container: whoami

    Browser->>Traefik: HTTP Request
    Traefik->>Whoami: Forward Request
    Whoami->>Traefik: Response
    Traefik->>Browser: HTTP Response
#+end_mermaid
...
sequenceDiagram
    ...
    RaspberryPi->>App: Forward Request
    App->>RaspberryPi: Response
 ...
 * Enter the following rules:
   ...
   * Public SFTP access:
     ...
     * Sources: All IPv4, All IPv6, or a specific set of static IP
       addresses if you want to be more secure.
     * Description: /Optional/ - this is so you can use [[/portable-docker/install-web-services/sftp/][SFTP]].
  * Click =Create Firewall=.
#+end_notice
...
You now need to be able to control the sentry's =root= user from the
 Raspberry Pi's =pi= user over SSH. 
...
#+attr_shortcode: :style tip :title Designate a single Docker admin machine
#+begin_notice
You may want to share a single sentry among several Docker servers at
various locations, and this is supported, but you should designate
only *one machine* as the main configuration point (Docker admin) for
all other contexts. In this example, the Raspberry Pi is the Docker
admin who controls all of your Docker servers, including the sentry
and itself.
#+end_notice
...
You can examine the list of your contexts, which you should have
three by now:
 ...
 * =default= is the default context created by Docker itself when it
   is installed. This context is *not used* by this configuration.
 * =localhost= is the SSH context of the sworkstation over localhost.
   This is the main context this configuration uses to configure your
   applications.
 * =sentry= is the SSH context of the sentry server, entirely remote
   controlled from the Pi.

#+attr_shortcode: :style tip :title Tip
#+begin_notice
The =*= next to the default context indicates the current context of
the =docker= command. But since you will be using the context aliases
=pi= and =sentry=, you won't be using the =docker= command directly,
so the =*= can be ignored.
#+end_notice
...
[[https://github.com/fail2ban/fail2ban?tab=readme-ov-file#readme][Fail2Ban]] scans log files and temporarily bans IP addresses that have
too many authentication attempts. Fail2Ban can be configured to
monitor the host SSH server and the Traefik access log, each with
configurable rules.
...
If you want to make sure your current IP address does not get banned,
add it to the default ignore list:
...
#+attr_shortcode: :style tip :title Tip
#+begin_notice
The IP ranges for localhost (=127.0.0.1/8=) and the Traefik wireguard
VPN (by default =10.13.16.0/24=) are added to this ignore list
automatically. You don't need to add these ones yourself.
#+end_notice
...
#+attr_shortcode: :style tip :title Tip
#+begin_notice
You may also create the route interactively through the [[/portable-docker/appendix/traefik-config-menu/#create-custom-routes][Traefik config
menu]].
#+end_notice
...
#+begin_notice
There is an important choice to make regarding how you want the
network of the Pi to be setup:
 ...
 * Do you still want to be able to access the Pi directly from your
   LAN, without needing to go through the sentry? Perhaps you won't
   always have internet access and you still want to access these
   services? If so, choose =No=.
 * The more secure option is to choose =Yes= and always go through the
   sentry. This will block access between the Pi and your LAN. You should 

Regardless of this choice, the SSH service will be provided to the
LAN, so you can change this setting at any time.
#+end_notice

Finish entering your wireguard client settings by copying them from
the temporary buffer you saved them in:
...
#+attr_shortcode: :style secondary :title Run this on your Raspberry Pi
#+begin_run
sentry route set --layer7 git.pi.example.com 10.13.16.2 443 --proxy-protocol true
#+end_run
...
Press =ESC= four times to quit the config tool.

**** Create a route from the forgejo entrytpoint to the Pi

#+attr_shortcode: :style secondary :title Run this on your Raspberry Pi
#+begin_run
sentry route set ssh --layer4 10.13.16.2 2222 --proxy-protocol false
#+end_run
...
[[/img/portable-docker/treasure-map.webp]]
...
#+attr_shortcode: :style primary :title Info
#+begin_notice
This chapter is focused soley on enhancing client authentication with
mTLS, but it still uses the Let's Encrypt CA for the server
certificates. If you don't want to rely upon Let's Encrypt, and you
want to run a fully self-hosted Public Key Infrastructure (PKI) for
all of your services, please see the appendix chapter [[/portable-docker/appendix/private-acme/][Private ACME]].
...
trusted header =X-Client-CN= for the purpose of secondary
...
#+attr_shortcode: :style secondary :title Run this on your Raspberry Pi
#+begin_run
sentry route set --layer7 ca.pi.example.com 10.13.16.2 443 --proxy-protocol true
#+end_run
...
client certificate, otherwise you will encounter the error =No
matching DNS names.=
#+end_notice
...
You should now receive one of the following errors, both indicating
that a client certificate is required:
...
:EXPORT_HUGO_WEIGHT: 660
:EXPORT_HUGO_SLUG: docker-registry
:END:

[[/img/portable-docker/registry.webp]]

A Docker registry is a service for storing and distributing Docker
images (OCI images). Public registries like [Docker
Hub](https://hub.docker.com/) are common, but private registries offer
...
#+attr_shortcode: :style secondary :title Run this on your Raspberry Pi
#+begin_run
sentry route set --layer7 registry.pi.example.com 10.13.16.2 443 --proxy-protocol true
#+end_run
...
#+attr_shortcode: :style secondary :title Run this on your Raspberry Pi
#+begin_run
sentry route set --layer7 immich.pi.example.com 10.13.16.2 443 --proxy-protocol true
#+end_run
...
subgraph Docker network
    C[Thttpd container] --|volume link|--> V((volume: thttpd_files))
    A[SFTP container] --|volume link|--> V((volume: thttpd_files))
end
#+end_mermaid
...
#+attr_shortcode: :style secondary :title Run this on your Raspberry Pi
#+begin_run
sentry route set --layer7 www.pi.example.com 10.13.16.2 443 --proxy-protocol true
#+end_run
...
#+attr_shortcode: :style secondary :title Run this on your Raspberry Pi
#+begin_run
sentry route set sftp --layer4 10.13.16.2 2223 --proxy-protocol false
#+end_run
...
#+begin_mermaid
sequenceDiagram
    participant Browser as FileStash UI - Web Browser
    participant FileStash as FileStash static server
    participant Minio as Minio S3 - Object store

    Browser->>FileStash: HTTP request for static app page
    FileStash->>Browser: Response with static app page
    Browser->>Minio: Direct API request for object data
    Minio->>Browser: Direct response with object data
#+end_mermaid
...
#+attr_shortcode: :style secondary :title Run this on your Raspberry Pi
#+begin_run
sentry route set --layer7 s3.pi.example.com 10.13.16.2 443 --proxy-protocol false --restart false
sentry route set --layer7 s3-console.pi.example.com 10.13.16.2 443 --proxy-protocol false --restart true
#+end_run
...
#+attr_shortcode: :style secondary :title Run this on your Raspberry Pi
#+begin_run
sentry route set --layer7 filestash-test.pi.example.com 10.13.16.2 443 --proxy-protocol true
#+end_run
...
#+attr_shortcode: :style secondary :title Run this on your Raspberry Pi
#+begin_run
sentry route set --layer7 homepage.pi.example.com 10.13.16.2 443 --proxy-protocol true --restart false
sentry route set --layer7 homepage-webhooks.pi.example.com 10.13.16.2 443 --proxy-protocol true --restart true
#+end_run
...
#+attr_shortcode: :style secondary :title Run this on your Raspberry Pi
#+begin_run
sentry route set --layer7 yourls.pi.example.com 10.13.16.2 443 --proxy-protocol true
#+end_run
...
#+attr_shortcode: :style secondary :title Run this on your Raspberry Pi
#+begin_run
sentry route set --layer7 nginx.pi.example.com 10.13.16.2 443 --proxy-protocol true
#+end_run
...
#+attr_shortcode: :style secondary :title Run this on your Raspberry Pi
#+begin_run
sentry route set --layer7 jupyterlab.pi.example.com 10.13.16.2 443 --proxy-protocol true
#+end_run
...
** Manual installation
:PROPERTIES:
:EXPORT_HUGO_SECTION_FRAG: manual-installation
:EXPORT_HUGO_WEIGHT: 10000
:END:
...
These are the manual steps you can take if you do not wish to run
[[/portable-docker/set-up-raspberry-pi/install-docker-and-d.rymcg.tech/][the
automated script]].
...
There are actually two automated scripts:
 ...
 1) The [[https://github.com/EnigmaCurry/d.rymcg.tech/blob/master/_scripts/bootstrap_sworkstation.sh][Debian version]], which is described earlier in this book.
 2) The [[https://github.com/EnigmaCurry/d.rymcg.tech/blob/master/_scripts/bootstrap_sworkstation_fedora.sh][Fedora version]], which is not covered in this book but is very
    similar to the Debian version.

This section won't use either of them, but I thought you might want to
know.
#+end_notice
...
*** Set up SSH
:PROPERTIES:
:EXPORT_FILE_NAME: 010110-set-up-ssh
...
*** Install Docker
:PROPERTIES:
:EXPORT_FILE_NAME: 010200-install-docker
:EXPORT_HUGO_WEIGHT: 10200
:EXPORT_HUGO_SLUG: install-docker
:END:

**** Install Docker

[[https://docs.docker.com/engine/install/][Follow the official guide to installing Docker Engine]].
...
#+end_run
  ...
  * If it's working, you should see a =Hello from Docker!= message and
    some other help info.
...
*** Install d.rymcg.tech
:PROPERTIES:
:EXPORT_FILE_NAME: 010300-install-d-rymcg-tech
:EXPORT_HUGO_WEIGHT: 10300
...
** Traefik config menu
:PROPERTIES:
:EXPORT_HUGO_SECTION_FRAG: traefik-config-menu
:EXPORT_HUGO_WEIGHT: 10050
:END:
...
Here are some extra things you can do in the Traefik config menu
system that were skipped in the main text of the book.

**** Create custom routes

The main way to create custom routes is with the contextual =route=
CLI command:

#+attr_shortcode: :style secondary :title Run this on your Raspberry Pi
#+begin_run
sentry route set --layer7 whoami.example.com 10.13.16.2 443 --proxy-protocol true
#+end_run
...
Another way you can create routes is through the interactive Traefik
config menu:
...
** Private ACME
:PROPERTIES:
:EXPORT_FILE_NAME: 010100-Private ACME
:EXPORT_HUGO_WEIGHT: 10100
:EXPORT_HUGO_SLUG: Private ACME
:END:
...
:PROPERTIES:
:EXPORT_FILE_NAME: 010200-Mutual TLS for Web and Mobile
:EXPORT_HUGO_WEIGHT: 10200
