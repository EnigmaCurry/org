...
even use the same IP address as before. It needs to be plug and play
with minimal disruption to the clients that expect this server to be
online and available. No matter what kind of internet connection I
have it connected to, it should be quick to get this server back
online.

** Sentry Droplet

To accomplish this feat, we need to set up another server (to be named
=sentry=) that has a static IP address and accessible from the public
internet. I use a DigitalOcean droplet, which is a type of Virtual
Private Server (VPS), which exists in the public cloud. The sentry is
always on and anyone in the world can connect to it (via HTTP).
However, the sentry does not serve any applications by itself; it only
creates the public Traefik proxy which forwards incoming connections
(ingress) through a backdoor VPN connection to the Raspberry Pi. The
Pi initiates this connection automatically whenever it boots and comes
online.
...
server, but rather remotely control it from your workstation. This is
one of the ways to follow the good DevOps practice, summed up by the
metaphor: "Treat your infrastructure like cattle, not like pets."
However, this strategy has the drawback that if you lose your
workstation, it makes it diffcult to adminster the server (you would
have to recreate your =.env= files on a new workstation, from backup,
or from scratch).

To avoid this complexity, and to make it easier to maintain after long
periods of inactivity, we will make an important compromise and an
...
runs Docker, rather than a separate workstation. That way you've got
everything together in one little box.
#+end_notice

This way, everything you need is installed on the Raspberry Pi itself.
To gain access from any other workstation, you only need to set up an
SSH client, along with an authorized key. You can login to the Pi and
perform all maintainance locally, without needing to manage a separate
workstation. Your portable Docker server is therefore a /pet/ Docker
server, with your "workstation" being located in the home directory of
the =pi= user, colocated on the Raspberry Pi itself.

**** Editing files on the Pi

One consequence of this choice is that unless you hook up a display
and keyboard to the Raspberry Pi, you'll eventually encounter the need
to edit some files remotely on the Pi. You have a few options in that
regard:
...
completion                    Set up TAB completion in your shell
...
[[/img/portable-docker/launch-droplet.webp]]
