 ...
 * Enter the following rules:
   ...
   * Public SSH access for Forgejo (public git access):
     ...
     * Sources: All IPv4, All IPv6, or a specific set of static IP
       addresses if you want to be more secure.
     * Description: /Optional/ - this is so you can access the SSH console of the
       Raspberry Pi through the public sentry.
  * Click =Create Firewall=.
#+end_notice
...
* Set up public SSH
:PROPERTIES:
:EXPORT_HUGO_SECTION_FRAG: set-up-public-ssh
...
Now that the VPN is active, you can create a public route to access
the SSH console of the Raspberry Pi from anywhere.
...
*** Create the public SSH entrypoint

The sentry is already running an SSH server on port =22=, so you need
to use a different port to access SSH on the Pi. This example will use
TCP port =2220=.

#+attr_shortcode: :style tip
#+begin_notice
Don't forget to open TCP port 2220 in the DigitalOcean firewall.
#+end_notice
...
ssh_pi  0.0.0.0  2220  tcp
#+end_stdout

#+attr_shortcode: :style warning :title Disable Proxy Protocol for the public entrypoint
#+begin_notice
Proxy protocol is not requried for any entrypoint on the sentry, so make sure it is disabled.
#+end_notice

Press =ESC= three times to go back to the main Traefik menu.
...
Select the =ssh_pi= entrypoint and enter the forwarding destination
(=10.13.16.2= which is the WireGuard VPN address of the Pi, port
=22=):
...
#+attr_shortcode: :style warning :title Disable Proxy Protocol for the public entrypoint
#+begin_notice
Make sure to disable [[https://www.haproxy.org/download/2.0/doc/proxy-protocol.txt][Proxy Protocol]], because SSH does not support it.
#+end_notice
...
accessing the Pi through the public sentry.
...
ryan     pts/2    10.13.16.1       18:24    0.00s  0.01s  0.01s w
#+end_stdout

#+attr_shortcode: :style tip
#+begin_notice
SSH does not support the [[https://www.haproxy.org/download/2.0/doc/proxy-protocol.txt][Proxy Protocol]], therfore there is no way to
forward the real IP address of the client. The IP address of the SSH
client will always appear to have the IP address of the sentry
(=10.13.16.1=). You may enable the Traefik access logs on the sentry
and record the real IP address of the client at that point.
#+end_notice
...
[[https://www.acme.com/software/thttpd/][Thttpd]] is a tiny static HTTP server and it will be used as a small
demonstration of the effect of transferring files with SFTP. This same
approach will work with any container's volume.
...
Repeat the same process on the sentry:
...
Unfortunately, the traditional tool =rsync= only works with SSH, not
SFTP. An alternative tool named [[https://rclone.org/][Rclone]] can be used instead.
