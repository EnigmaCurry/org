...
online and available. No matter what kind of connection to the
Internet it has available, it should be quick to get this server back
online.
...
public internet. I use a DigitalOcean droplet, which is a type of
Virtual Private Server (VPS), which exists in the public cloud. The
sentry is always on and anyone in the world can connect to it (via
HTTP). However, the sentry does not serve any applications by itself;
it only creates the public [[https://doc.traefik.io/traefik/][Traefik]] proxy which forwards incoming
connections (ingress) through a backdoor VPN connection to the
Raspberry Pi. The Pi initiates this connection automatically whenever
it boots and comes online.
...
The cloud server (sentry) forwards incoming connections from the
Internet to the Raspberry Pi over the WireGuard VPN connection.
Traefik is used on the cloud server to create routes for your
services. Traefik supports the following route types:

 * HTTP Layer 7 (Routes a domain name [TLS SNI] + HTTP path over port
   443 to a backend HTTP server)
 * TCP Layer 4 (Routes a TCP ADDRESS:PORT to a specific
