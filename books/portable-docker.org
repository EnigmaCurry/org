 ...
 * UDP Layer 4 - TODO (unimplemented, but similar to TCP)
...
workstation/ and urges you to never actually log in to your Docker
...
lose access to your workstation, it makes it diffcult to adminster the
...
#+begin_quote
Your portable Pi is treated as a /pet/ Docker server (not cattle), you
must name it and interact with it directly via SSH console.
#+end_quote

If you reference any other d.rymcg.tech documentation, and it's
talking about "your workstation", for the purposes of this book, you
should understand that this means for you to use the =pi= user on the
Raspberry Pi, not your personal workstation.
...
ranching) workstation to control all of them, and you'd have to login
to each one of them individually and configure them directly. However,
...
Unless you want to carry a display and keyboard to plug directly into
the Raspberry Pi, you're still going to need to travel with another
computer (e.g., a laptop) and set that up with an SSH key on it, so
that you can remotely log in to the Pi.

You can use any sort of personal computer, as long as its capable of
...
different options in that regard:
 ...
 1. Learn how to use one of the many terminal mode text editors (e.g.,
    Emacs, Vim, nano, etc.) and edit the files through an SSH console
 ...
 2. Edit the files directly on your personal workstation and set up
    your editor so that it that saves files transparently over SSH
    (e.g., Emacs' [[https://www.gnu.org/software/tramp/][TRAMP]], VS Code's
    [[https://marketplace.visualstudio.com/items?itemName=ms-vscode-remote.remote-ssh][Remote
    SSH]], Vim's [[https://www.vim.org/scripts/script.php?script_id=1075][Netrw]] etc, or universally via [[https://wiki.archlinux.org/title/SSHFS][sshfs]].)
...
You can have as many peers as you want, use commas to seprate the
names. Multiple peers can share the same sentry.
...
#+attr_shortcode: :title Run this on the Raspberry Pi :style secondary
#+begin_run
sentry make traefik logs service=wireguard
#+end_run
...
#+attr_shortcode: :title Run this on the Raspberry Pi :style secondary
#+begin_run
sentry make traefik shell service=wireguard-client
#+end_run
...
fork of [[https://github.com/go-gitea/gitea?tab=readme-ov-file#gitea][Gitea]], which is a fork of [[https://github.com/gogs/gogs][Gogs]]. Installing Forgejo is useful
 ...
 * Providing an OAuth2 identity service for your organization,
   facilitating single-sign on to all of your apps via Traefik
   [[https://doc.traefik.io/traefik/middlewares/http/forwardauth/][forward-auth middleware]] ([[https://github.com/EnigmaCurry/d.rymcg.tech/tree/master/traefik-forward-auth#readme][traefik-forward-auth]]).
 ...
 * You should use the =root= account for creating new OAuth2 apps and creating new users.
