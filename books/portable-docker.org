...
reccomendation to do so on a separate workstation. This way you've got
everything together in one little portable box. If you had a whole
fleet of Raspberry Pis to manage, this would be a terrible idea, but
you will likely start this journey with only one Pi, so this is a good
initial compromise to make. If you want to build a proper workstation,
read the [[/linux-workstation/][Linux Workstation]] book and the main [[https://github.com/enigmacurry/d.rymcg.tech?tab=readme-ov-file#readme][d.rymcg.tech README]].
#+end_notice

Unless you want to plug a display and keyboard into the Pi, you still
need to travel with another computer (e.g., laptop) and to set up an
SSH key on it to remotely log in to the Pi, but thats it. Log in to
the Pi via SSH and perform all configuration and maintainance, without
needing to manage a separate workstation. Your portable Docker server
is therefore a /pet/ Docker server plus workstation, with all the
configuration sitting in the home directory of the =pi= user.
