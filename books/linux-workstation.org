...
#+hugo_weight: auto
#+hugo_paired_shortcodes: %notice badge button %children %index %run %stdout %edit math mermaid openapi %env
#+STARTUP: align
...
You can bend the workstation rules a bit, and run some servers in
virtual machines (VM). Keeping your servers in isolated VMs will keep
your workstation clean and server free. You can treat VMs just like
any remote Linux host, and access them over SSH.
...
*** Run the rest of these commands as the libvirt-admin user

#+attr_shortcode: :style info
#+begin_notice
*You need to run the rest of the commands in this chapter running as
the =libvirt-admin= this user.*
...
*** Create server VM from cloud image

**** Configure temporary variables for a Debian VM config:

#+begin_env
CLOUD_IMAGE=https://cloud.debian.org/images/cloud/bookworm/latest/debian-12-genericcloud-amd64.qcow2
NAME=debian
OS_VARIANT=debian12
MEMORY=1024
CPUS=2
