 ...
 * Wait a minute for the VM to boot and bring up its network interface.
 
**** Find the IP address of the VM (named debian-dev)

#+attr_shortcode:  :style secondary :title Run this as the libvirt-admin user
#+begin_run
MAC_ADDRESS=$(virsh domiflist ${NAME:-debian-dev} | grep virbr0 | grep -Po ".* \K.*$")
IP_ADDRESS=$(arp -n | grep $MAC_ADDRESS | cut -d " " -f 1)
echo ""
echo ${MAC_ADDRESS}
echo ${IP_ADDRESS}
echo ""
echo "# Copy this comamnd and run it as root to make a static DHCP lease:"
echo "sudo virsh net-update default add ip-dhcp-host \"&lt;host mac='${MAC_ADDRESS}' name='${NAME}' ip='${IP_ADDRESS}' /&gt;\" --live --config"
#+end_run

#+attr_shortcode: :style tip
#+begin_notice
The final =echo= command prints the command that you need to run as
*root* (sudo) to add the static DHCP lease:

#+begin_run
## Adds static DHCP lease to libvirt dnsmasq:
## Change the NAME, MAC, and IP addresses to suit your VM!
sudo virsh net-update default add ip-dhcp-host \
  "&lt;host mac='${MAC_ADDRESS}' name='${NAME}' ip='${IP_ADDRESS}' /&gt;" \
  --live --config
#+end_run

This can also be done by editing the config file manually:
...
&lt;network&gt;
  &lt;name&gt;default&lt;/name&gt;
  &lt;uuid&gt;bcffa07e-02de-4b64-8820-783b92c80a6a&lt;/uuid&gt;
&lt;network&gt;
  &lt;name&gt;default&lt;/name&gt;
  &lt;uuid&gt;bcffa07e-02de-4b64-8820-783b92c80a6a&lt;/uuid&gt;
  &lt;forward mode='nat'/&gt;
  &lt;bridge name='virbr0' stp='on' delay='0'/&gt;
  &lt;mac address='52:54:00:82:1d:f8'/&gt;
  &lt;ip address='192.168.122.1' netmask='255.255.255.0'&gt;
    &lt;dhcp&gt;
      &lt;range start='192.168.122.2' end='192.168.122.254'/&gt;
      &lt;host mac='00:60:2f:39:8e:5f' name='debian-dev' ip='192.168.122.165'/&gt;
    &lt;/dhcp&gt;
  &lt;/ip&gt;
&lt;/network&gt;
#+end_stdout
