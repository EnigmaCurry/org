...
(although a workstation could also be VPS that you SSH into, this book
...
The role of a workstation should be very different than that of a
server. A workstation's only purpose is to serve you, the user, in the
moment while you are interfacing with its physical keyboard/mouse/etc
interface. A workstation /is/ usually connected to a network, but only
as a client, not as a server. (Of course, you may bend this rule if
you like, to make your computer a server-workstation or
"Sworkstation", but it is cleaner, and more secure, to use separate
machines for these very different roles.)
...
host. From your normal workstation account, you can access the VM's
=root= shell, over (local) SSH conntection.

These instructions will cover installing libvirt, and creating a
barebones Debian or Fedora VM (but any cloud-init image will work),
