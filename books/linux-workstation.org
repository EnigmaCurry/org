...
#+begin_run
curl https://raw.githubusercontent.com/solokeys/solo2-cli/main/70-solo2.rules | sudo tee /etc/udev/rules.d/solokey2.rules
#+end_run
...
The workaround is either to do so using the chromium browser (which
you may need to install in a toolbox container):

#+begin_run
chromium
#+end_run

 * Navigate to =chrome://settings/securityKeys=
 * Click =Create a PIN=.
