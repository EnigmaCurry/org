...
*** Temporary unit
#+attr_shortcode: :style root :title Run this as root on your workstation
#+begin_run
NAME=debian-dev
(set -e
cat << EOF > /etc/systemd/system/libvirt-VM-${NAME}.service
[Unit]
Description=Start kernel virtual machine: ${NAME}

[Service]
Type=oneshot
ExecStartPre=sh -c "virsh net-start default || true"
ExecStart=su libvirt-admin -c "virsh start ${NAME}"

[Install]
WantedBy=multi-user.target
EOF
systemctl daemon-reload
systemctl enable --now libvirt-VM-${NAME}.service
systemctl status libvirt-VM-${NAME}.service
)
#+end_run
...
sudo python -m venv /usr/local/src/virsh-start-stop/env
sudo /usr/local/src/virsh-start-stop/env/bin/pip install libvirt-python
)
#+end_run
...
ExecStart=su libvirt-admin -c "/usr/bin/python /usr/local/src/virsh-start-stop/src/virsh_start_stop/virsh_start_stop.py --machine %i --state started"
ExecStop=su libvirt-admin -c "/usr/bin/python /usr/local/src/virsh-start-stop/src/virsh_start_stop/virsh_start_stop.py --machine %i --state stopped --timeout 80"
...
*** Enable service to autostart each VM
...
*** OK what if autostart didn't work?
...
:PROPERTIES:
:EXPORT_FILE_NAME: vm-management
:END:
...
NAME=debian-dev
(set -e
cat << EOF > /etc/systemd/system/libvirt-DNAT-${NAME}.service
[Unit]
Description=DNAT port forwarding for kernel virtual machine: ${NAME}
After=network.target
Wants=network.target

[Service]
Type=oneshot
ExecStart=/etc/libvirt/hooks/qemu ${NAME} start
...
#+begin_run
NAME=debian-dev
sudo su libvirt-admin -c "virsh shutdown ${NAME}"
#+end_run
