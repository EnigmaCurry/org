...
** Get your Solokey ready

*** Purchase at least two Solokeys

[[https://www.crowdsupply.com/solokeys/somu#products][They come in lots of different sizes.]]

*** Programming the Solokey

Follow the [[https://docs.solokeys.dev/programming/][Programming instructions]]
...
requirements for using =sudo=. This chapter will install [[https://developers.yubico.com/pam-u2f/][pam_u2f]],
which enables PAM authentication via FIDO2/U2F compatible hardware
tokens like the [[https://solokeys.com/][Solokey]]. =sudo= will then require a Solokey button
press /and/ a password each time it requires authentication.
...
*** Open a root session as anti-lockout measure
...
#+begin_run
## Repeat this step for each solokey:
pamu2fcfg >> ${TMP_KEYS} && \
     echo >> ${TMP_KEYS}
#+end_run
 ...
 * Unplug the second solokey and repeat again for however many other
   solokeys you want to register.
 
 * When you've written all the keys to =${TMP_KEYS}=, reformat and
   install them into their final destination =/etc/u2f_mappings=:
...
*** Create custom PAM config for U2F

This includes the required settings:
...
directly after the =#%PAM-1.0= header. Your solokey is going to be the
/first required/ authentication mechanism, and the existing password
flow is the /second required/ authentication method.
...
#+attr_shortcode: :style tip
#+begin_notice
The line that reads =auth include u2f-required=, on line 2, is the
only line that was added to this file. All the rest of the lines were
here from the original package.
#+end_notice
