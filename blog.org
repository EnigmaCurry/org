...
VMs in [part 3](/blog/linux/mutable-vms/); it's *stateless*. To change
...
That builds a fresh rootfs from your current Nix config and re-injects
the identity files - effectively a brand new container. It's safe to run
anytime, because the bind-mounted datasets are **created if missing but
never destroyed on teardown** - the data lives on the host's ZFS pool,
not in the container, and outlives any number of recreates. Nothing on
the NAS itself is precious; the pool is.
...
The rootfs is read-write, so you *can* `ssh` in and run `sudo
nixos-rebuild switch` against the seeded `/etc/nixos` flake for a quick
